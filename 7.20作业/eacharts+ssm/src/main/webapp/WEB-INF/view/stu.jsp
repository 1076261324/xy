...
<body class="gray-bg">
        ...
        <div class="row">
			  <div class="col-lg-2">
				<div class="input-group">
			      ...
			      <input type="text" name="stuSname" class="form-control" id="txt_search_username" >
				</div>
			  </div>
			  <div class="col-lg-2">
				<div class="input-group">
					<span class="input-group-addon">所属学校</span>
					<select class="form-control" name="stuSid" id = "txt_search_roleid">
						<option value="0">---请选择---</option>
						<c:forEach items="${slist }" var="s">
						 	<option value="${s.stuSid }">${s.schSname }</option>
						</c:forEach>
                	</select>
				</div>
			 </div>
			...
			<div class="modal-content">
				<div class="modal-body">
					<form id="form_user" method="post" action="reserveStu.htm">
						<input type="hidden" name="stuId" id="hidden_txt_userid" value=""/>
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								<td>学生姓名：</td>
								<td><input type="text" id="stuSname" name="stuSname"
									class="form-control" aria-required="true" required/></td>
								<td>&nbsp;&nbsp;</td>
								<td>学生生日：</td>
								<td><input type="date" id="stuBirthday" name="stuBirthday"
									class="form-control" aria-required="true" required/></td>
							</tr>
							<tr>
								<td>学生年龄：</td>
								...
								<td>学生性别：</td>
								<td>
									<input type="radio" id="stuGender" name="stuGender" value="男" aria-required="true" required/>男
									<input type="radio" id="stuGender" name="stuGender" value="女" aria-required="true" required/>女
								</td>
							</tr>
							<tr>
								<td>所属学校：</td>
								<td colspan="4">
									<select class="form-control" name="stuSid" id = "stuSid" aria-required="true" required>
	...
	<div class="modal fade" id="modal_user_import" role="dialog" aria-labelledby="modal_user_import" aria-hidden="true">
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
						<form action="importStu.htm" id="form_import" method="post" enctype="multipart/form-data">
	...
	var TableInit = function () {
	    ...
	    oTableInit.queryParams = function (params) {
	        var temp = {//这里的键的名字和控制器的变量名必须一致，这边改动，控制器也需要改成一样的
	            ...
	            stuSname: $("#txt_search_username").val(),
	            stuSid: $("#txt_search_roleid").val(),
	    			...
	    			$.ajax({
	    			    ...
	    			    success:function(res){
	    			    	...
	    			    	if(res.success){
		    			    	...
		    			    	res.stulist.forEach(function(student){
		    			    		xdata.push(student.sname);
		    			    		ydata.push({value:student.num,name:student.sname});
		    			    	})
		    			    	//柱形图
				option = {
		    			    		    title: {
	...
	function initEditUser(getSelection){
		$('#hidden_txt_userid').val(getSelection.stuId);
		$('#stuSname').val(getSelection.stuSname);
		$('#stuBirthday').val(new Date(getSelection.stuBirthday).Format("yyyy-MM-dd"));
		$('#stuAge').val(getSelection.stuAge);
		$('#stuSid').val(getSelection.stuSid);
		$("input[value='"+getSelection.stuGender+"']").prop("checked",true);
